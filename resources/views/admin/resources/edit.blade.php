            ...
            <form method="POST" action="{{ route('admin.resources.update', [$resource->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}"
                        name="description" id="description">{!! old('description', $resource->description) !!}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type"
                        ...
                        @foreach (App\Models\Resource::TYPE_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('type', $resource->type) === (string) $key ? 'selected' : '' }}>
                                {{ $label }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('type'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2" name="level_id" id="level_id" required>
                        @foreach ($levels as $id => $entry)
                            <option value="{{ $id }}" {{ (old('level_id') == $id || $resource->lesson->level->id == $id) ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('level'))
                ...
                <div class="form-group">
                    ...
                    <select
                        ...
                        @foreach ($subjects as $id => $entry)
                            <option value="{{ $id }}" {{ (old('subject_id') == $id || $resource->lesson->subject->id == $id) ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('lesson') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($lessons as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('lesson_id') ? old('lesson_id') : $resource->lesson->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text" name="link"
                        id="link" value="{{ old('link', $resource->link) }}">
                    @if ($errors->has('link'))
                        <div class="invalid-feedback">
                            {{ $errors->first('link') }}
                        </div>
...
@section('scripts')
    <script defer>
        ...
        window.selectedLevel = {{ $resource->lesson->level->id }};
        window.selectedSubject = {{ $resource->lesson->subject->id }};
        ...
        $(document).ready(function() {
			...
			$('#lesson_id').val({{ $resource->lesson->id }});
        })
    </script>
    <script>
