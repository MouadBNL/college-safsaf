    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.subjects.update", [$subject->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('label') ? 'is-invalid' : '' }}" type="text" name="label" id="label" value="{{ old('label', $subject->label) }}" required>
                @if($errors->has('label'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('levels') ? 'is-invalid' : '' }}" name="levels[]" id="levels" multiple>
                    @foreach($levels as $id => $level)
                        <option value="{{ $id }}" {{ (in_array($id, old('levels', [])) || $subject->levels->contains($id)) ? 'selected' : '' }}>{{ $level }}</option>
                    @endforeach
                </select>
